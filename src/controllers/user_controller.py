...
user_ns = Namespace("user", description= "User operations.", authorizations=authorizations)
...
class UserResource(Resource):
    ...
    @user_ns.doc("Update A User", security="JWTTokenAuth")
    ...
    @user_ns.doc("Delete A User", security="JWTTokenAuth")
    @role_required(roles=[Roles.admin.value])
    def delete(self, id):
...
class UserListResource(Resource):
    @user_ns.doc("Get User List", security="JWTTokenAuth")
    @user_ns.response(200,"Get Success",model= [user])
    @role_required(roles=[Roles.admin.value])
    def get(self):
        return get_all_users(email_confirmed=False)
    
    @user_ns.doc("Create A User", security="JWTTokenAuth")
